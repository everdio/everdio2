...
namespace Component\Core\Adapter {
    ...
    trait Unix {
        ...
        public function syntax(string $thread) {
            if (!\str_contains(($response = \exec("php -l " . $thread)), "No syntax errors detected")) {
                return (string) $response;
            }
        }

        public function kill($pid, $signal): void {
            if (\posix_getpgid($pid)) {
                \posix_kill($pid, $signal);
            }
        }

        public function exec(string $thread, int $sleep = 0, int $timeout = 300, bool|string $output = false): int {
            if (empty($output)) {
                $output = "/dev/null";
            }
