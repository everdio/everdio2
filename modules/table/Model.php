    ...
    final class Model extends \Component\Core\Adapter\Mapper\Model {
        ...
        public function setup() : void {
            ...
            foreach ($columns->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                $sample = $this->prepare(\sprintf("SELECT`%s`FROM`%s`.`%s`WHERE`%s`IS NOT NULL ORDER BY CHARACTER_LENGTH(`%s`)DESC LIMIT 1", $row["COLUMN_NAME"], $this->database, $this->table, $row["COLUMN_NAME"], $row["COLUMN_NAME"]));
                $value = (empty($row["COLUMN_DEFAULT"]) && ($sample && $sample->execute()) ? $this->desanitize($sample->fetchColumn()) : $row["COLUMN_DEFAULT"]);                
            ...
            foreach($many->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                //$this->parents = [$this->labelize($row["COLUMN_NAME"]) => $this->namespace . "\\" . $this->labelize($row["TABLE_NAME"])];
            } 
        }
    }
}