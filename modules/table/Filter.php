    ...
    final class Filter extends \Component\Validation {
        ...
        public function __construct(array $mappers, string $operator = "and", string $expression = "=", array $operators = []) {
            foreach ($mappers as $mapper) {
                if ($mapper instanceof \Component\Core\Adapter\Mapper && isset($mapper->mapping) && \sizeof($mapper->restore($mapper->mapping))) {            
                    foreach ((isset($this->primary) && \sizeof($mapper->restore($mapper->primary)) === \sizeof($mapper->primary) ? $mapper->restore($mapper->primary) : $mapper->restore($mapper->mapping)) as $parameter => $value) {
