...
namespace Modules\Table\Model {
    ...
    class SQLite extends \Modules\Table\Model {
        ...
        public function setup(): void {
            ...
            foreach ($columns->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                $sample = $this->prepare(\sprintf("SELECT`%s`FROM`%s`WHERE`%s`IS NOT NULL ORDER BY CHARACTER_LENGTH(`%s`)DESC LIMIT 1", $row["COLUMN_NAME"], $this->resource, $row["COLUMN_NAME"], $row["COLUMN_NAME"]));
