...
namespace Modules\Node {
    ...
    final class Find extends \Component\Validation {
        ...
        public function __construct(string $xpath, array $validations = [], string $wrap = "(%s)") {
            $xparts = $parts = \explode(\DIRECTORY_SEPARATOR, $this->_clean($xpath));

            foreach ($validations as $validation) {
                if ($validation instanceof \Component\Validation && $validation->isValid()) {
                    if (\array_key_exists(($end = \array_key_last(\array_intersect(($fparts = \explode(\DIRECTORY_SEPARATOR, ($fpath = $this->_clean(($filter = $validation->execute()))))), $parts))), $xparts)) {
